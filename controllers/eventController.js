...
controller.addEvent = (req, res) => {
    const { selectedBusiness, startDate, endDate, eventName, description } = req.body

    connection.query(`INSERT INTO event (event_name, event_description, startDate, endDate, business_id) VALUES ('${eventName}', '${description}', '${startDate}', '${endDate}', '${selectedBusiness}');`, (err, result) => {
        if (err) throw err
...
controller.getEventFromUser = (req, res) => {
    ...
    WHERE business.user_id = ${id};`, (err, result) => {
        if (err) throw err
...
controller.getEventFromBusiness = (req, res) => {
    ...
    connection.query(`SELECT * FROM event WHERE business_id = ${businessId};`, (err, result) => {
        if (err) throw err
...
controller.getEventInfo = (req, res) => {
    ...
    connection.query(`SELECT * FROM event WHERE event_id = '${eventId}';`, (err, result) => {
        if (err) throw err
...
controller.updateEvent = (req, res) => {
    ...
    connection.query(`UPDATE event SET ${event_name ? `event_name='${event_name}',` : ""}${event_description ? `event_description='${event_description}',` : ""}${startDate ? `startDate='${startDate}',` : ""}${endDate ? `endDate='${endDate}'` : ""} WHERE event_id = ${event_id}`, (err, result) => {
        if (err) throw err
...
controller.getFutureEvents = (req, res) => {
    connection.query(`SELECT * FROM event WHERE endDate > now()`, (err, result) => {
        if (err) throw err
