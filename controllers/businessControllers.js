...
controller.update = (req, res) => {
  ...
  connection.query(
    `UPDATE login SET ${password ? `password='${password}',` : ""}${
      email ? ` email='${email}'` : ""
    } WHERE id=${userId}`,
    (err, result) => {
      if (err) throw err;
...
controller.updateBusiness = (req, res) => {
  ...
  connection.query(
    ...
    }${address ? ` address='${address}'` : ""} WHERE id=${id}`,
    (err, result) => {
      if (err) throw err;
...
controller.insertBusiness = (req, res) => {
  ...
  connection.query(
    `INSERT INTO business (businessName,
        ...
        postcode,
        instagram,
        user_id) VALUES ('${businessName}', '${address}', '${description}', '${category}', '${telephone}', '${email}', '${lat}', '${lon}', '${city}', '${postcode}', '${instagram}', '${id}');`,
    (error, result) => {
...
controller.getBusinessFromUser = (req, res) => {
  ...
  connection.query(
    `SELECT * FROM business WHERE user_id = ${id}`,
    (error, result) => {
      if (error) throw error;
...
controller.getCategories = (req, res) => {
 ...
 WHERE
    TABLE_SCHEMA = 'doggiesintown' AND TABLE_NAME = 'business' AND COLUMN_NAME = 'category';`,
    (error, results) => {
      res.send(
        results[0].categoryType
          .replace("enum('", "")
          .replace("')", "")
          .split("','")
      );
...
controller.setMainPhoto = (req, res) => {
  ...
  connection.query(
    `UPDATE business SET mainImagePath = '${req.file.filename}' WHERE (id = '${business_id}');`,
    (err, result) => {
      if (err) throw err;
...
controller.getBusinessMap = (req, res) => {
  const { latBottom, latTop, lonLeft, lonRight, category } = req.body;
  console.log(category);

  connection.query(
    `SELECT business.* FROM business LEFT JOIN login ON business.user_id = login.id WHERE (login.iban IS NULL OR login.iban = '') AND lat > ${latBottom} AND lat < ${latTop} AND lon > ${lonLeft} AND lon < ${lonRight}${
      category !== "null" && category ? ` AND category = '${category}'` : ""
    }`,
    (err, result) => {
      if (err) throw err;
...
controller.getBusinessPremium = (req, res) => {
  ...
  connection.query(
    `SELECT business.* FROM business LEFT JOIN login ON business.user_id = login.id WHERE login.iban<>'' AND lat > ${latBottom} AND lat < ${latTop} AND lon > ${lonLeft} AND lon < ${lonRight}${
      category !== "null" && category ? ` AND category = '${category}'` : ""
    }`,
    (err, result) => {
      if (err) throw err;
