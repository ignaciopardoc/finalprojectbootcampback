...
controller.setReview = (req, res) => {
    ...
    connection.query(`SELECT * FROM review WHERE dog_id = ${dog_id} AND business_id = ${business_id}`, (error, check) => {
        if (error) throw error

        else if (check.length === 0){
            connection.query(`INSERT INTO review (stars, ${review ? "review," : ""} dog_id, business_id) VALUES ('${stars}', ${review ? `'${review}', ` : ""} '${dog_id}', '${business_id}')`, (err, result) => {
                if (err) throw err
