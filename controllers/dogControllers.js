...
controller.insertDog = (req, res) => {
    ...
    connection.query(`INSERT INTO dog (name, user_id, breed) VALUES ('${name}', '${id}', '${breed}')`, (err, result) =>{
        if (err) throw err
...
//Add or update a dog photo
controller.setMainPhoto = (req, res) => {
    const { dog_id } = req.params
    console.log(req.file.filename)
    connection.query(`UPDATE dog SET photo = '${req.file.filename}' WHERE (id = '${dog_id}');`, (err, result) => {
        if (err) throw err
...
//Obtain the info from one dog
controller.getDogInfo = (req, res) => {
    let token = req.headers.authorization.replace("Bearer ", "")
    console.log(token)
    const { id } = jwt.verify(token, myPrivateKey)
...
controller.updateDog = (req, res) => {
    ...
    const {name, description, breed, sex} = req.body
   connection.query(`UPDATE dog SET ${breed ? `breed='${breed}',` : ""} ${description ? `description='${description}',` : ""}${sex ? `sex='${sex}',` : ""}${name ? `name='${name}'` : ""} WHERE id='${dog_id}'`, (err, result) => {
       if (err) throw err
