...
controller.insertDog = (req, res) => {
  ...
  connection.query(
    `INSERT INTO dog (name, user_id, breed) VALUES ('${name}', '${id}', '${breed}')`,
    (err, result) => {
      if (err) throw err;
...
controller.setMainPhoto = (req, res) => {
  ...
  connection.query(
    `UPDATE dog SET photo = '${req.file.filename}' WHERE (id = '${dog_id}');`,
    (err, result) => {
      if (err) throw err;
...
controller.updateDog = (req, res) => {
  ...
  connection.query(
    `UPDATE dog SET ${breed ? `breed='${breed}',` : ""} ${
...
controller.getOneDogInfo = (req, res) => {
  ...
  connection.query(
    `SELECT * FROM dog WHERE id = '${dog_id}'`,
    (err, result) => {
      if (err) throw err;
